"""Top-level pipeline: load → shell → seeds → cells → cut → write."""
...
def run(
    ...
    # Sharp edges (--soft-edge-angle) partition the surface into smooth
    # patches. Seeds are distributed PER PATCH (proportional to patch area,
    # minimum one per non-trivial patch). The per-patch Voronoi labelling
    # requires every smooth patch to have at least one seed; plain
    # Poisson sampling on the whole surface routinely leaves small patches
    # unseeded, which would force global-Euclidean fallback assignments
    # and produce broken cells that span patch boundaries.
    ...
    # Mesh to use for cell building: the same one the seeds were drawn
